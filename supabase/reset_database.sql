-- =============================================================================
-- RESET DATABASE â€” drops ALL app tables (v1 + v2) and recreates v2 schema
-- =============================================================================
-- WARNING: This deletes ALL ingredients, products, recipes, settings, and history.
-- Auth users are kept â€” sign in again; enterprise membership is recreated at the end.
--
-- Run this entire file once in the Supabase SQL Editor.
-- =============================================================================

-- Break circular FK before drops (no-op if table missing)
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ingredients') THEN
    ALTER TABLE ingredients DROP CONSTRAINT IF EXISTS fk_ingredients_active_vendor_price;
  END IF;
END $$;

-- v2 tables
DROP TABLE IF EXISTS product_variety_selling_price_history CASCADE;
DROP TABLE IF EXISTS product_base_recipe_ingredients CASCADE;
DROP TABLE IF EXISTS product_recipe_versions CASCADE;
DROP TABLE IF EXISTS product_variety_ingredients CASCADE;
DROP TABLE IF EXISTS product_variety_acceptances CASCADE;
DROP TABLE IF EXISTS product_variety_cost_history CASCADE;
DROP TABLE IF EXISTS product_varieties CASCADE;
DROP TABLE IF EXISTS ingredient_vendor_price_history CASCADE;
DROP TABLE IF EXISTS ingredient_price_history CASCADE;
DROP TABLE IF EXISTS catalogue_templates CASCADE;
DROP TABLE IF EXISTS ingredient_vendor_prices CASCADE;
...
-- old v1 helper functions
DROP FUNCTION IF EXISTS update_ingredient_timestamp() CASCADE;
...
-- Bootstrap default enterprise for existing auth users after reset
...
BEGIN
  ...
  INSERT INTO enterprise_members (enterprise_id, user_id, role)
  SELECT default_ent_id, u.id,
    CASE WHEN u.id = owner_user_id THEN 'owner' ELSE 'editor' END
  FROM auth.users u
  ON CONFLICT (enterprise_id, user_id) DO NOTHING;
END $$;
